...
class DefenderPretrainPsroRunner(RunnerBase):
    """
    Runner that trains a defender policy via PPO as a best response to a fixed attacker runner.
    """
    ...
    def __init__(
        ...
        # Note: joint legal-action mask is computed online in _legal_mask_from_info.
    ...
    def _compute_action_space(self) -> tuple[int, int, int]:
        ...
        action_dim = max_branch ** env.num_defenders
    ...
    def _build_full_action_map(self) -> tuple[list[list[int]], torch.Tensor]:
        ...
        defender_count = base_env.num_defenders
        base_env.close()
        choices = [list(range(self.max_branching)) for _ in range(defender_count)]
    ...
    def _map_branch_to_node_actions(self, legal_actions: list[list[int]], branch_actions: list[int]) -> list[int]:
        ...
        if len(branch_actions) != len(trimmed_actions):
            raise ValueError(
                f"Branch action length {len(branch_actions)} does not match defender count {len(trimmed_actions)}"
            )
        ...
        for idx, (branch_idx, acts) in enumerate(zip(branch_actions, trimmed_actions)):
            if branch_idx < 0 or branch_idx >= len(acts):
                raise ValueError(f"Invalid branch index {branch_idx} for defender {idx} with {len(acts)} legal actions")
    ...
    def policy_action_batch(self, obs_list: list[dict], info_list: list[dict]) -> list[list[int]]:
        defender_actions_list, _ = self._defender_step_batch(obs_list, info_list, need_stats=False)
        return defender_actions_list
    ...
    def collect(
        ...
        # num_envs = max(1, int(vec_envs or 1))
    ...
    def _format_ppo_batch(self, batch: list[Any]) -> list[dict[str, Any]]:
        """
        Flatten and normalize rollout records to the dict-per-step format
        expected by PPOAlgorithm.update.

        Supports nested episode lists or dictionaries containing a "steps"/"episode_steps" key.
        """
        if not batch:
            return []
        # Already flat list of step dicts
    ...
    def evaluate(self, opponent_runner, episodes: int = 2) -> dict[str, Any]:
        ...
        for _ in range(episodes):
            ...
            while not done:
                attacker_actions = opponent_runner.policy_action(obs, info)
                defender_actions = self.policy_action(obs, info)
                ...
                if done:
                    total += 1
                    if reward["defender"] > reward["attacker"]:
                        wins += 1
        env.close()
        return {"win_rate": wins / max(total, 1)}

    def _choose_attacker_runner(self, attacker_runners: list[Any], meta_strategy: Any | None) -> Any:
        ...
        if meta_strategy is None:
            ...
            if probs.shape[0] != len(attacker_runners):
                raise ValueError("meta_strategy length must match attacker_runners length")
            total = probs.sum()
            if total <= 0:
                probs = np.ones(len(attacker_runners), dtype=float) / float(len(attacker_runners))
            else:
    ...
    def run(
        ...
        try:
            for batch_idx in range(train_batches):
                ...
                # sps = episodes_collected / max(rollout_time, 1e-9)
                # logger.info(
                #     "Defender rollout batch %s: episodes=%s, time=%.3fs, SPS=%.2f",
                #     batch_idx,
                #     episodes_collected,
                #     rollout_time,
                #     sps,
    ...
    def pretrain(
        ...
        vec_envs: int | None = None,
        log_prefix: str | None = "pretrain",
        save_path: str | None = None,
    ) -> dict[str, Any]:
        metrics: dict[str, Any] = {}
        if num_iterations <= 0 or num_tasks <= 0 or episodes_per_task <= 0:
            return metrics

        num_envs = min(episodes_per_task, int(vec_envs or 1))
        pool = VecRolloutPool(self.env_builder, num_envs=num_envs)
        try:
            for iteration in range(num_iterations):
                iteration_batches: list[Any] = []
                for _ in range(num_tasks):
                    if hasattr(attacker_runner, "clone"):
                        task_runner = attacker_runner.clone()
                    else:
                        task_runner = copy.deepcopy(attacker_runner)
                    if not hasattr(task_runner, "random_policy"):
                        raise NotImplementedError("Attacker runner must implement random_policy for pretraining")
                    task_runner.random_policy()
                    task_batch = self.collect(
                        [task_runner],
                        meta_strategy=None,
                    ...
                    iteration_batches.extend(task_batch)

                ppo_batch = self._format_ppo_batch(iteration_batches)
                metrics = self.algorithm.update(ppo_batch, self.agent)
                if log_prefix and metrics:
                    prefixed = {f"{log_prefix}/{k}": v for k, v in metrics.items()}
                else:
    ...
    def clone(self, agent_state_dict=None) -> "DefenderPretrainPsroRunner":
