...
class policy_net(nn.Module):
    def __init__(self, game, args):
        ...
        training_horizon = getattr(args, "defender_training_time_horizon", None)
        self.time_horizon = game.time_horizon if training_horizon is None else int(training_horizon)
    ...
    def forward(self, obs, legal_act):
        with torch.no_grad():
            ...
            t_index = min(len(attacker_his) - 1, self.time_horizon)
            t = F.one_hot(torch.tensor(t_index), num_classes=self.time_horizon+1).to(self.args.device).float()
            obs=torch.cat([obs,t])
            obs=self.obs_f1(obs)
    ...
    def batch_forward(self, def_pos, att_pos, time, legal_act):
        ...
        time = torch.clamp(time, max=self.time_horizon)
        t = F.one_hot(time, num_classes=self.time_horizon+1).float()
...
class value_net(nn.Module):
    ...
    def forward(self, obs):
        with torch.no_grad():
            ...
            t_index = min(len(attacker_his) - 1, self.time_horizon)
            t = F.one_hot(torch.tensor(t_index),
