...
import numpy as np
import torch
from graphchase.solver.nsgzero.network import *
from graphchase.solver.nsgzero.mcts import MCTS
...
class NFSPAttacker:
    def __init__(self, game, args):
        ...
        self.paths = paths_by_exit
        self._all_paths = all_paths
        self.reachable_exits = [exit_node for exit_node in self.exits if self.paths.get(exit_node)]
        if not self.reachable_exits:
    ...
    def update(self, act, val):
        if getattr(self.args, "reward_mode", "utility") == "win_rate":
    ...
    def reset(self, train=True):
        ...
        self.is_br=True
        value = []
        for key in self.reachable_exits:
            value.append(self.act_est[key])
        q_value = [math.exp(i) for i in value]
        sum_value = sum(q_value)
        strategy = [q / sum_value for q in q_value]
        selected_exit=np.random.choice(self.reachable_exits, 1, p=np.array(strategy)).item()
        candidate_paths = self.paths.get(selected_exit, [])
    ...
    def _next_path_node(self) -> int:
        if self.t >= len(self.path):
            return int(self.path[-1])
        return int(self.path[self.t])
...
class RandomAttacker:
    def __init__(self, game, args):
        ...
        _, all_paths = game.build_paths_by_exit(cutoff=game.time_horizon)
    ...
    def reset(self, train=None):
        self.t = 1
        if not self.paths:
            self.path = [self.attacker_init[0]]
        else:
...
#         if args.graph_id ==1:
#             self.paths=torch.load("/home/ami/wanqi/NSGZero_LSX/graph file/10*10_1vs4/evader_location_55_time_horizon_6_path_num_84.pkl")[1]
#         elif args.graph_id ==2:
#             self.paths=torch.load("/home/ami/wanqi/NSGZero_LSX/graph file/20*20_1vs2/evader_location_229_time_horizon_10_path_num_178.pkl")[1]
#         elif args.graph_id ==3:
#             self.paths=torch.load("/home/ami/wanqi/NSGZero_LSX/graph file/20*20_1vs4/evader_location_210_time_horizon_11_path_num_129.pkl")[1]
#         elif args.graph_id ==4:
#             self.paths=torch.load("/home/ami/wanqi/NSGZero_LSX/graph file/50*50_1vs4/evader_location_1225_time_horizon_26_path_num_1404.pkl")[1]
#         elif args.graph_id ==5:
#             self.paths=torch.load("/home/ami/wanqi/NSGZero_LSX/graph file/100*100_1vs8/evader_location_5050_time_horizon_51_path_num_598.pkl")[1]
