...
class Defender_RL_DRRN(nn.Module):
    def __init__(self, num_nodes, time_horizon, embedding_size=32, num_kernels=128, kernel_size=[3, 8],
                 hidden_size=64, relevant_v_size=64, if_gate=True, naive=False, if_gru=False, num_defender=1):
        super(Defender_RL_DRRN, self).__init__()
        if not if_gru:
            self.seq_encoder = Gated_CNN(
        ...
        self.fc1 = nn.Linear(num_kernels+hidden_size +
                             relevant_v_size, relevant_v_size)
        self.fc2 = nn.Linear(relevant_v_size, 1)
        ...
        self.naive = naive
        if self.naive:
            self.fc_f = nn.Linear(num_kernels+hidden_size, relevant_v_size)
        self.num_defender = num_defender
    ...
    def forward(self, states, actions):
        ...
        if h_feature.dim() == 2:
            s_vector = torch.cat((h_feature, p_feature), dim=1)
        else:
            s_vector = torch.cat((h_feature, p_feature.squeeze()), dim=0)

        if self.naive:
            # shape:(batch, relevant_v_size)
            s_vector = F.relu(self.fc_f(s_vector))

        single_sample = False
        if s_vector.dim() == 1:
            single_sample = True
            s_vector = s_vector.unsqueeze(0)
        ...
        # if self.num_defender==1:
        #     actions = [torch.LongTensor(k).to(device) for k in actions]
        # elif self.num_defender>1:
        actions = [torch.LongTensor(k).to(device) for k in actions]
        actions = pad_sequence(actions, batch_first=True,
                               padding_value=0).detach()
        actions.requires_grad = False  # shape:(batch, max_num_actions)

        # shape:(batch, max_num_actions, embedding_size)
        #a_vector = self.embedding_a(actions)

        a_vector = self.embedding_a(actions)
        if self.num_defender > 1:
            a_vector = torch.flatten(a_vector, start_dim=2)

        # shape:(batch, max_num_actions, hidden_size)
        a_vector = F.relu(self.fc_a_1(a_vector))
        # shape:(batch, max_num_actions, relevant_v_size)
        a_vector = F.relu(self.fc_a_2(a_vector))

        if not self.naive:
            s_vector = s_vector.unsqueeze(1).repeat(1, a_vector.size(1), 1)

            # shape(batch, max_num_actions, full_features)
            feature = torch.cat((s_vector, a_vector), dim=2)
            q_val = F.relu(self.fc1(feature))
            q_val = self.fc2(q_val).squeeze(2)
        else:
            q_val = torch.matmul(a_vector, s_vector.unsqueeze(
                2)).squeeze(2)  # shape:(batch, max_num_actions)

        if not single_sample:
            q_val += mask

        optimal_q, idx = torch.max(q_val, dim=1)
        optimal_a = actions[torch.arange(actions.size(0)), idx]

        # optimal_a/_q shape:(batch,) ; q_val shape:(batch, max_num_actions)
        return optimal_a, optimal_q, q_val.squeeze(0)
...
class Defender_SL_DRRN(nn.Module):
    def __init__(self, num_nodes, time_horizon, embedding_size=32, num_kernels=128, kernel_size=[3, 8],
                 hidden_size=64, relevant_v_size=64, if_gru=False, num_defender=1):
        super(Defender_SL_DRRN, self).__init__()
        if not if_gru:
            self.seq_encoder = Gated_CNN(
                num_nodes, time_horizon, embedding_size, num_kernels, kernel_size, True)
        else:
            self.seq_encoder = Encoder_GRU(
                num_nodes, embedding_size, num_kernels)
        self.embedding_p = self.seq_encoder.embedding
    ...
    def forward(self, states, actions):
        ...
        # shape(batch, max_num_actions, full_features)
        feature = torch.cat((s_vector, a_vector), dim=2)
        logis = F.relu(self.fc1(feature))
        logis = self.fc2(logis).squeeze(2)

        if not single_sample:
            logis += mask

        return logis.squeeze(0)
...
class Defender_SL_MA(nn.Module):
    def __init__(self, max_actions, num_nodes, time_horizon, embedding_size=32, num_kernels=256, kernel_size=[3],
                 hidden_size=64, if_gate=True, if_gru=False, num_defender=1):
        super(Defender_SL_MA, self).__init__()
        if not if_gru:
            self.seq_encoder = Gated_CNN(
                num_nodes, time_horizon, embedding_size, num_kernels, kernel_size, if_gate)
        else:
        ...
        self.fc_p_1 = nn.Linear(embedding_size*num_defender, hidden_size)
        self.fc_p_2 = nn.Linear(hidden_size, hidden_size)
        self.fc1 = nn.Linear(num_kernels+hidden_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, max_actions)
        self.to(device)
        self.num_defender = num_defender
        self.num_nodes = num_nodes

    def forward(self, inputs):
        # inputs should be in the form [([1,2,3],2),([2,3,4,4],4)]
        attacker_history, position = zip(*inputs)
        if self.num_defender > 1:
            assert len(position[0]) == self.num_defender
        elif self.num_defender == 1:
            assert isinstance(position[0], int)
...
class Attacker_RL_DRRN(nn.Module):
    def __init__(self, num_nodes, time_horizon, embedding_size=32, num_kernels=128, kernel_size=[3, 8],
                 hidden_size=64, relevant_v_size=64, if_gate=True, naive=False, if_gru=False):
        super(Attacker_RL_DRRN, self).__init__()
        if not if_gru:
            self.seq_encoder = Gated_CNN(
                num_nodes, time_horizon, embedding_size, num_kernels, kernel_size, if_gate)
        else:
            self.seq_encoder = Encoder_GRU(
                num_nodes, embedding_size, num_kernels)
        self.fc_s = nn.Linear(num_kernels, relevant_v_size)
        self.embedding_a = nn.Embedding(
            num_nodes+1, embedding_size, padding_idx=0)
        self.fc_a_1 = nn.Linear(embedding_size, hidden_size)
        self.fc_a_2 = nn.Linear(hidden_size, relevant_v_size)

        self.fc1 = nn.Linear(2*relevant_v_size, relevant_v_size)
        self.fc2 = nn.Linear(relevant_v_size, 1)
    ...
    def forward(self, states, actions):
        s_vector = self.seq_encoder(states)  # shape:(batch, num_kernels)
        s_vector = self.fc_s(s_vector)  # shape:(batch, relevant_v_size)
...
class Attacker_SL_DRRN(nn.Module):
    def __init__(self, num_nodes, time_horizon, embedding_size=32, num_kernels=128, kernel_size=[3, 8],
                 hidden_size=64, relevant_v_size=64, if_gru=False):
        super(Attacker_SL_DRRN, self).__init__()
        if not if_gru:
            self.seq_encoder = Gated_CNN(
                num_nodes, time_horizon, embedding_size, num_kernels, kernel_size, True)
        else:
...
class Attacker_SL_MA(nn.Module):
    def __init__(self, max_actions, num_nodes, time_horizon, embedding_size=32, num_kernels=256, kernel_size=[3],
                 hidden_size=64, if_gate=True, if_gru=False):
        super(Attacker_SL_MA, self).__init__()
        if not if_gru:
            self.seq_encoder = Gated_CNN(
                num_nodes, time_horizon, embedding_size, num_kernels, kernel_size, if_gate)
        else:
    ...
    def forward(self, inputs):
        x = self.seq_encoder(inputs)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
        return x


def test_defender(single_sample=False):
    state = [([1, 2, 3], 2), ([2, 3, 4, 3], 5)]
    actions = [[1, 2, 3], [4, 5, 6, 9, 8]]
    if single_sample:
        state = [([1, 2, 3], 2)]
        # actions=[[3]]
        actions = [[1, 2, 3, 4]]

    # q_val output shape: [batch,max_num_actions]/ [num_actions]
    RL_DRRN = Defender_RL_DRRN(num_nodes=10, time_horizon=30, num_defender=1)
    RL_MA = Defender_MA_DQN(max_actions=10, num_nodes=10,
                            time_horizon=10, num_defender=1)
    # output shape: [batch,num_nodes]/ [num_nodes]
    SL_DRRN = Defender_SL_DRRN(num_nodes=10, time_horizon=30, num_defender=1)
    SL_MA = Defender_SL_MA(max_actions=10, num_nodes=10,
                           time_horizon=10, num_defender=1)

    o_a, o_q, q = RL_DRRN(state, actions)
    print('RL_DRRN output is ', 'Optimal actions :', o_a, '\n \n', 'Optimal_values :',
          o_q, '\n \n', 'Full Q values : \n', q)

    print('\n RL_MA output is {}'.format(RL_MA(state, actions)))

    sl_output = SL_DRRN(state, actions)
    print('\n \n SL_DRRN ouput is {}'.format(sl_output))

    sl_output = SL_MA(state)
    print('\n \n SL_MA ouput is {}'.format(sl_output))


def test_multi_defender(single_sample=False):
    state = [([1, 2, 3], (2, 3)), ([2, 3, 4, 3], (1, 5))]
    actions = [[(4, 5)], [(2, 3), (2, 5), (2, 3), (4, 3)]]
    if single_sample:
        state = [([1, 2, 3], (2, 3))]
        # actions=[[3]]
        actions = [[(4, 5), (3, 4)]]
    ...
    # q_val output shape: [batch,max_num_actions]/ [num_actions]
    RL_DRRN = Defender_RL_DRRN(num_nodes=10, time_horizon=30, num_defender=2)
    RL_MA = Defender_MA_DQN(max_actions=10, num_nodes=10,
                            time_horizon=10, num_defender=2)
    # output shape: [batch,num_nodes]/ [num_nodes]
    SL_DRRN = Defender_SL_DRRN(num_nodes=10, time_horizon=30, num_defender=2)
    SL_MA = Defender_SL_MA(max_actions=10, num_nodes=10,
                           time_horizon=10, num_defender=2)
...
def test_attacker(single_sample=False):
    state = [[1, 2, 3], [4, 5, 6, 9]]
    actions = [[1, 2, 3], [4, 5, 6, 9, 1]]
    if single_sample:
        state = [[1, 2, 3]]
        actions = [[1, 2, 3, 4]]

    # q_val output shape: [batch,max_num_actions]/ [num_actions]
    RL_DRRN = Attacker_RL_DRRN(num_nodes=10, time_horizon=30)
    RL_MA = Attacker_MA_DQN(max_actions=10, num_nodes=10,
                            time_horizon=10)
    # output shape: [batch,num_nodes]/ [num_nodes]
    SL_DRRN = Attacker_SL_DRRN(num_nodes=10, time_horizon=30)
    SL_MA = Attacker_SL_MA(max_actions=10, num_nodes=10,
                           time_horizon=10)
...
def test_encoder_speed():
    gated_cnn = Gated_CNN(800, 100, 128, 512, [5])
    gru = Encoder_GRU(800, 128, 512)
    ...
    print('gated cnn time : {:.4f}s'.format(end-start))

    start = time.time()
    for k in range(100):
        _ = gru(inputs)
    end = time.time()
    print('gru time : {:.4f}s'.format(end-start))


# if __name__ == "__main__":

#     print('\n ------------------------------------------Single Defender----------------------------------------------- \n')
#     test_defender(single_sample=False)
#     test_defender(single_sample=True)

#     print('\n ------------------------------------------Multi Defender----------------------------------------------- \n')
#     test_multi_defender(single_sample=False)
#     test_multi_defender(single_sample=True)

#     print('\n ------------------------------------------Single Attacker----------------------------------------------- \n')
#     test_attacker(single_sample=False)
#     test_attacker(single_sample=True)
