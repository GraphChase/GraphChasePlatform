    ...
    def __init__(self, log_path="./logging_data", name="run"):
        ...
        for i in range(1000):
            ...
            if not os.path.exists(name):
                break
        self.run_name = name
        logger.info("TBLogger initialized: %s", self.run_name)

    def note(self, metrics, step=None):
        if step is None:
            step = self.last_step
        logger.info("TBLogger[%s] step=%s metrics=%s", self.run_name, step, metrics)
        self.last_step = step

    def finish(self):
        logger.info("TBLogger finished: %s", self.run_name)
