...
class RHMAPPOPolicy:
    ...
    def get_actions(self, pooled_node_emb, T, cent_obs, obs, action_mask=None, batch=False):
        # Compute actions and value function predictions for the given inputs.
        actions, action_log_probs = self.actor(pooled_node_emb, T, obs, action_mask=action_mask, batch=batch)
    ...
    def evaluate_actions(self, pooled_node_emb, T, cent_obs, obs, action, action_mask=None, batch=False):
    ...
    def act(self, pooled_node_emb, T, obs, action_mask=None, batch=False):
        # Compute actions using the given inputs.
        actions, _ = self.actor(pooled_node_emb, T, obs, action_mask=action_mask, batch=batch)
        return actions
...
class RHMAPPOPolicyScratch:
    ...
    def __init__(self, mappo_args, args, obs_space, cent_obs_space, act_space, device=torch.device("cpu")):
        ...
        if args.use_emb_layer:
            ...
            raise ValueError("Only support architecture with state representation layer.")
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.lr, eps=self.opti_eps, weight_decay=self.weight_decay)
        self.critic_optimizer = torch.optim.Adam(self.critic.parameters(), lr=self.critic_lr, eps=self.opti_eps, weight_decay=self.weight_decay)

    def get_actions(self, hgs, T, cent_obs, obs, action_mask=None, batch=False):
        # Compute actions and value function predictions for the given inputs.
        actions, action_log_probs = self.actor(hgs, T, obs, action_mask=action_mask, batch=batch)
    ...
    def evaluate_actions(self, hgs, T, cent_obs, obs, action, action_mask=None, batch=False):
        # Get action logprobs / entropy and value function predictions for actor update.
        action_log_probs, dist_entropy, action_probs = self.actor.evaluate_actions(
            hgs,
            T,
    ...
    def act(self, hgs, T, obs, action_mask=None, batch=False):
        # Compute actions using the given inputs.
        actions, _ = self.actor(hgs, T, obs, action_mask=action_mask, batch=batch)
        return actions
