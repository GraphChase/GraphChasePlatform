...
# from grasper_ppo.layers import GraphConvolution
...
#
#     def forward(self, x, adj, frozen=False):
#         if frozen:
#             with torch.no_grad():
...
class HyperNetwork(nn.Module):
    """
        1. no node embedding: use raw state as plicy input, pooled node embedding + T as hyper input
        2. use node embedding: i) query node embedding through raw state as policy input, ii) pooled node embedding + T as hyper input
    """
