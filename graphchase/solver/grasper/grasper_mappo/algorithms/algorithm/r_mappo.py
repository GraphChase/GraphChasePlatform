...
from ...utils.util import get_gard_norm, huber_loss
...
class RMAPPO:
    ...
    def __init__(self, mappo_args, args, policy, device=torch.device("cpu")):
        ...
        if args.update_every_n_episodes > 0:
            self._step = int(args.checkpoint / args.update_every_n_episodes)
        else:
    ...
    def ppo_update(self, sample, update_actor=True):
        share_obs_batch, obs_batch, action_masks_batch, actions_batch, value_preds_batch, return_batch, masks_batch, old_action_log_probs_batch, adv_targ, demo_act_probs_batch, pooled_node_embs_batch = sample
        values, action_log_probs, dist_entropy, log_probs = self.policy.evaluate_actions(
            share_obs_batch,
            obs_batch,
            actions_batch,
    ...
    def train(self, buffer, update_actor=True, train_num_per_ite=1):
        ...
        if total_transition_num > buffer.batch_size:
            ...
            for _ in range(train_num_per_ite):
                ...
                for start in range(0, total_transition_num, mini_batch_size):
                    batch_indices = rand[start:start + mini_batch_size]
                    sample = buffer.get_batch(advantages, self.device, indices=batch_indices)
