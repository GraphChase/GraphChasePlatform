...
class H_Actor(nn.Module):
    ...
    def forward(self, pooled_node_emb, T, obs, action_mask=None, batch=False):
    ...
    def evaluate_actions(self, pooled_node_emb, T, obs, action, action_mask=None, batch=False):
        actor_features = self.base(pooled_node_emb, T, obs, batch=batch)
        actor_features = _mask_action_logits(actor_features, action_mask)
        probs = F.softmax(actor_features, dim=-1)
        m = Categorical(probs)
...
class H_Actor_With_Emb_Layer_Scratch(nn.Module):
    ...
    def forward(self, hgs, T, obs, action_mask=None, batch=False):
    ...
    def evaluate_actions(self, hgs, T, obs, action, action_mask=None, batch=False):
        actor_features = self.base(hgs, T, obs, batch=batch)
        actor_features = _mask_action_logits(actor_features, action_mask)
        probs = F.softmax(actor_features, dim=-1)
        m = Categorical(probs)
