...
class Attacker(object):
    ...
    def get_strategy(self, info, time):
        if time == 1:
            strategy = np.zeros(info.action_number) + 1.0 / float(info.action_number)
        else:
            x = self.info_to_state(info)
    ...
    def train_regret_network(self, lr, time, train_epoch=2000, batch_size=64):
        ...
        train_x = [i[0] for i in self.regret_training_data[:]]
        train_action = [i[1] for i in self.regret_training_data[:]]
        train_y = [i[2] for i in self.regret_training_data[:]]
...
def regret_to_strategy(regret, action_number):
    ...
    for a in range(action_number):
        strategy[a] = max(regret[a], 0)
        strategy_sum += strategy[a]
    if strategy_sum > 0:
        strategy /= strategy_sum
    else:
        strategy = np.zeros(action_number) + 1.0 / float(action_number)
    return strategy
