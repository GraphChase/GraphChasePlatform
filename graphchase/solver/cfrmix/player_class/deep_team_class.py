...
class DefenderGroup(object):
    ...
    def regret_memory_add(self, info, available_action, regret):
        ...
        for idx, action in enumerate(available_action):
            ...
            if self.reservoir_flag:
                self.reservoir_record(
                    "regret", [np.array(obs_1_list), np.array(obs_2_list), action_list, [regret[idx]]]
                )
            else:
                self.regret_training_data.append(
                    [np.array(obs_1_list), np.array(obs_2_list), action_list, [regret[idx]]]
                )
            ...
            if self.argumentation_flag:
                ...
                if self.reservoir_flag:
                    self.reservoir_record(
                        "regret",
                        [np.array(obs_1_list_temp), np.array(obs_2_list_temp), action_list_temp, [regret[idx]]],
                    )
                else:
                    self.regret_training_data.append(
                        [np.array(obs_1_list_temp), np.array(obs_2_list_temp), action_list_temp, [regret[idx]]]
    ...
    def get_strategy(self, info, agent_location, agent_avail_action, time):
        if time == 1:
            ...
            for i in range(len(agent_location)):
                strategy.append(np.zeros(len(agent_avail_action[i])) + 1.0 / float(len(agent_avail_action[i])))
        else:
    ...
    def train_regret_network(self, lr, time, train_epoch=2000, batch_size=64):
        print("number of defender regret training data", len(self.regret_training_data))
        train_x_1 = [i[0] for i in self.regret_training_data[:]]
        train_x_2 = [i[1] for i in self.regret_training_data[:]]
        train_action = [i[2] for i in self.regret_training_data[:]]
        train_y = [i[3] for i in self.regret_training_data[:]]

        optimizer = torch.optim.SGD(self.regret_model.parameters(), lr)
        criterion = torch.nn.MSELoss()

        if torch.cuda.is_available():
            train_x_1 = torch.tensor(np.array(train_x_1)).cuda().float()
            train_x_2 = torch.tensor(np.array(train_x_2)).cuda().float()
            train_action = torch.tensor(np.array(train_action)).cuda().float()
            train_y = torch.tensor(np.array(train_y)).cuda().float()
            criterion = criterion.cuda()
        else:
            train_x_1 = torch.tensor(np.array(train_x_1)).float()
            train_x_2 = torch.tensor(np.array(train_x_2)).float()
            train_action = torch.tensor(np.array(train_action)).float()
            train_y = torch.tensor(np.array(train_y)).float()

        torch_dataset = Data.TensorDataset(train_x_1, train_x_2, train_action, train_y)
        loader = Data.DataLoader(dataset=torch_dataset, batch_size=batch_size, shuffle=True, num_workers=0)

        for epoch in range(train_epoch):
            epoch_total_loss = 0

            for step, (batch_x_1, batch_x_2, batch_action, batch_y) in enumerate(loader):
                predict_y = self.regret_model(batch_x_1, batch_x_2, batch_action)
            ...
            if epoch == 0 or (epoch + 1) % 1000 == 0:
                print("Training defender regret model, ", "Epoch", epoch, ", Training epoch loss", epoch_total_loss)
    ...
    def train_strategy_network(self, lr, time, train_epoch=2000, batch_size=64):
        ...
        criterion = torch.nn.MSELoss()
        print("number of defender strategy training data", len(self.strategy_training_data))
        train_x = [i[0] for i in self.strategy_training_data[:]]
        train_obs = [i[1] for i in self.strategy_training_data[:]]
        train_action = [i[2] for i in self.strategy_training_data[:]]
        train_y = [i[3] for i in self.strategy_training_data[:]]

        if torch.cuda.is_available():
            train_x = torch.tensor(np.array(train_x)).cuda().float()
            train_obs = torch.tensor(np.array(train_obs)).cuda().float()
            ...
            train_obs = torch.tensor(np.array(train_obs)).float()
            train_action = torch.tensor(train_action).float()
            train_y = torch.tensor(train_y).float()

        torch_dataset = Data.TensorDataset(train_x, train_obs, train_action, train_y)
        loader = Data.DataLoader(dataset=torch_dataset, batch_size=batch_size, shuffle=True, num_workers=0)

        for epoch in range(train_epoch):
            epoch_total_loss = 0

            for step, (batch_x, batch_obs, batch_action, batch_y) in enumerate(loader):
                predict_y = self.strategy_model(batch_x, batch_obs, batch_action)
                batch_loss = criterion(predict_y, batch_y)
                optimizer.zero_grad()
                batch_loss.backward()
    ...
    def regret_to_strategy(self, regret, action_set):
        regret_list = np.array(regret)
        strategy_list = []
        index = 0
        for i, _ in enumerate(action_set):
            strategy = regret_list[index : index + len(action_set[i])]
            index += len(action_set[i])
            ...
            if total > 0:
                ...
                strategy = np.zeros(len(action_set[i]))
