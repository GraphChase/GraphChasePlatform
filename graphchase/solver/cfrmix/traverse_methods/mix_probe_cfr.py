...
from graphchase.solver.cfrmix.game_setting.information_set import (
    get_mix_information_set,
    is_terminal,
    run_action,
    terminal_util,
)
    ...
    if len(one_history) % 2 == player_id:
        if player_id == 0:
            ...
            for i, action_index in enumerate(action_set):
                ...
                if action_index in action_sampled_index:
                    util_for_action[i] = probe_traverse_tree(
                        graph, next_history, player_id, player_list, time_horizon, sample_number, itr_round
                    )
                else:
        ...
        if 1 - player_id == 0:
            ...
            for i, location in enumerate(one_history[-2]):
                ...
                player_list[1 - player_id].strategy_memory_add(
                    info, location, info.action_set[i], strategy_list[i] * itr_round
                )
