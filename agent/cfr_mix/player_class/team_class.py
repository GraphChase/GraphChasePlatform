...
class DefenderGroup(object):
    ...
    def train_regret_network(self, lr, time, train_epoch=2000, batch_size=64):
        ...
        train_obs_1 = [i[0] for i in self.regret_training_data[:]]
        train_obs_2 = [i[1] for i in self.regret_training_data[:]]
        train_action = [i[2] for i in self.regret_training_data[:]]
        train_regret = [i[3] for i in self.regret_training_data[:]]
    ...
    def train_strategy_network(self, lr, time, train_epoch=2000, batch_size=32):
        ...
        train_x1 = [i[0] for i in self.strategy_training_data[:]]
        train_x2 = [i[1] for i in self.strategy_training_data[:]]
        ...
        if torch.cuda.is_available():
            ...
            target_model = copy.deepcopy(self.strategy_model).cuda()
            criterion = criterion.cuda()
        else:
            ...
            target_model = copy.deepcopy(self.strategy_model)
