...
class DefenderRegretMixNet(torch.nn.Module):
    def __init__(self, obs_input_dim, player_number, hidden_dim):
        ...
        self.agent_model = torch.nn.ModuleList([AgentNet(obs_input_dim, hidden_dim, 4, 1) for _ in range(player_number)])
    ...
    def forward(self, obs_1_list, obs_2_list, action_list):
        ...
        for player in range(self.n_player):
            ...
            agent_regret = self.agent_model[player](obs_1, obs_2, action)
...
class DefenderGroup(object):
    def __init__(self, time_horizon, player_number, hidden_dim, reservoir=False, argumentation=False):
        ...
        self.strategy_training_data = [[] for _ in range(self.player_number)]
        ...
        self.strategy_model = torch.nn.ModuleList([AgentStrategyNet(self.input_dim, hidden_dim, 4, 1) for _ in range(self.player_number)])
    ...
    def reservoir_record(self, type, data, player_id):
        if type == 'regret':
            ...
            if len(self.strategy_training_data[player_id]) < self.memory_size:
                self.strategy_training_data[player_id].append(data)
            else:
                m = random.randint(1, self.strategy_data_number)
                if m <= self.memory_size:
                    self.strategy_training_data[player_id][m - 1] = data
    ...
    def strategy_memory_add(self, info, player_id, agent_location, agent_avail_action, strategy):
        ...
        for i, action in enumerate(agent_avail_action):
            if self.reservoir_flag:
                ...
                self.strategy_training_data[player_id].append([x, np.array([agent_location]), [action], [strategy[i]]])

    def get_strategy(self, info, agent_location, agent_avail_action, time):
        strategy_list = []
        for i in range(self.player_number):
            strategy = self.get_agent_strategy(info, i, agent_location[i], agent_avail_action[i], time)
            strategy_list.append(strategy)
        return strategy_list

    def get_agent_strategy(self, info, id, agent_location, agent_action, time):
        if time == 1:
            strategy = np.zeros(len(agent_action)) + 1. / float(len(agent_action))
        else:
            ...
            if torch.cuda.is_available():
                ...
                prediction = self.regret_model.agent_model[id](state, obs, action).squeeze(1).detach().numpy()
            strategy = self.regret_to_strategy(prediction, agent_action)
        return strategy

    def get_average_strategy(self, info, agent_location, agent_avail_action):
        strategy_list = []
        for i in range(self.player_number):
            strategy = self.get_agent_average_strategy(info, i, agent_location[i], agent_avail_action[i])
            strategy_list.append(strategy)
        return strategy_list

    def get_agent_average_strategy(self, info, player_id, agent_location, agent_avail_action_set):
        s = self.info_to_state(info)
        o = [agent_location]
        state, obs, ac = [], [], []
        for a in agent_avail_action_set:
            state.append(s)
            obs.append(o)
            ac.append([a])
        ...
        if torch.cuda.is_available():
            ...
            obs = torch.tensor(np.array(obs)).float()
            action = torch.tensor(np.array(ac)).float()
            prediction = self.strategy_model[player_id](state, obs, action).squeeze(1).detach().numpy()
        strategy = self.regret_to_strategy(prediction, agent_avail_action_set)
        return strategy

    # def regret_to_strategy_1(self, regret, avail_actions):
    ...
    def regret_to_strategy(self, regret, avail_actions):
        regret_list = np.array(regret)
        strategy = np.where(regret_list > 0, regret_list, 0)
        total = float(sum(strategy))
        if total > 0:
            strategy = strategy / total
        else:
            max_index = list(strategy).index(max(strategy))
            strategy = np.zeros(len(avail_actions))
            strategy[max_index] = 1
            # strategy_list.append(np.zeros(len(avail_actions[i])) + 1. / float(len(avail_actions[i])))
        return strategy
    ...
    def train_regret_network(self, lr, time, train_epoch=2000, batch_size=64):
        ...
        if torch.cuda.is_available():
            ...
            target_model = copy.deepcopy(self.regret_model).cuda()
            ...
            target_model = copy.deepcopy(self.regret_model)
        ...
        for epoch in range(train_epoch):
            ...
            for step, (batch_obs_1, batch_obs_2, batch_action, batch_y) in enumerate(loader):
                ...
                if time == 1:
                    zero = torch.zeros_like(batch_y)
                    batch_y = torch.where(batch_y < 0, zero, batch_y)
                    batch_loss = criterion(predict_y, batch_y)
                else:
                    before_y = target_model(batch_obs_1, batch_obs_2, batch_action)
                    label_y = (pow(time-1, 0.5) * before_y + batch_y)/pow(time, 0.5)
                    zero = torch.zeros_like(label_y)
                    label_y = torch.where(label_y < 0, zero, label_y)
                    batch_loss = criterion(predict_y, label_y)
                optimizer.zero_grad()
                batch_loss.backward()
    ...
    def train_strategy_network(self, lr, time, train_epoch=2000, batch_size=32):
        ...
        for i in range(self.player_number):
            print("number of strategy training data", len(self.strategy_training_data[i]))
            optimizer = torch.optim.Adam(self.strategy_model[i].parameters(), lr)
            train_x1 = np.array([i[0] for i in self.strategy_training_data[i][:]])
            train_x2 = np.array([i[1] for i in self.strategy_training_data[i][:]])
            train_action = np.array([i[2] for i in self.strategy_training_data[i][:]])
            train_y = np.array([i[3] for i in self.strategy_training_data[i][:]])
            ...
            if torch.cuda.is_available():
                ...
                target_model = copy.deepcopy(self.strategy_model[i]).cuda()
                criterion = criterion.cuda()
            else:
                ...
                target_model = copy.deepcopy(self.strategy_model[i])
            ...
            for epoch in range(train_epoch):
                ...
                for step, (batch_x1, batch_x2, batch_action, batch_y) in enumerate(loader):
                    predict_y = self.strategy_model[i](batch_x1, batch_x2, batch_action)
                    if time == 1:
                        zero = torch.zeros_like(batch_y)
                        batch_y = torch.where(batch_y < 0, zero, batch_y)
                        batch_loss = criterion(predict_y, batch_y)
                    else:
                        before_y = target_model(batch_x1, batch_x2, batch_action)
                        label_y = ((time - 1) * before_y + batch_y) / time
                        zero = torch.zeros_like(label_y)
                        label_y = torch.where(label_y < 0, zero, label_y)
                        batch_loss = criterion(predict_y, label_y)
                    optimizer.zero_grad()
                    batch_loss.backward()
                    torch.nn.utils.clip_grad_norm_(self.strategy_model[i].parameters(), 1)
                    optimizer.step()
                    epoch_total_loss += batch_loss.data
                if epoch == 0 or (epoch + 1) % 1000 == 0:
