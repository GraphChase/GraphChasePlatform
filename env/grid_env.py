...
class GridEnv(BaseGame):
    def __init__(self,
                 Graph: BaseGraph, 
                 nextstate_as_action = False,
        ...
        self._colums = Graph.column
        self._rows = Graph.row
        ...
        # 动作空间0-4，代表不动，上下左右，5个操作
        # defender 根据动作空间产生下一个位置的坐标
        ...
        self.pix_square_width = round((self.window_size - 20) / (self._colums-1),2)
        self.pix_square_height = round((self.window_size - 20) / (self._rows-1), 2)
        self.point_positions = []
        for j in range(self._rows):
            for i in range(self._colums):
                # calculate the positon of each point
                x = round(10 + i * self.pix_square_width, 2)
                y = round(10 + j * self.pix_square_height, 2)
    ...
    def step(self, actions: np.ndarray,) -> tuple[np.ndarray, list, bool, bool, dict]:
        ...
        if not self.nextstate_as_action:
            directions = np.array([self._action_to_direction[action] for action in actions])
            next_states = current_states + directions
        else:
            next_states = actions
